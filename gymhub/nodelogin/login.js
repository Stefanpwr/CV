...
app.post('/signup', function (request, response) {
    ...
    response.redirect('/account');
    //response.sendFile('/account.html',{ root : 'D:/pwr/6 semestr/ZTW/ztw_bend-node/public'});
    response.end();
...
app.post('/changePas', function (request, response) {
    if(!session.loggedin){
        ...
        //response.send('Change password done');
        response.redirect('/account');
        response.end();
    }
...
app.post('/changeMail', function (request, response) {
    if(!session.loggedin){
        ...
        session.username = emailn;
        response.redirect('/account');
        //response.send('Change email done');
        response.end();
    }
